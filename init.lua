...
-- Better indenting, don't lose selection when shifting
vim.keymap.set('v', '<', '<gv')
vim.keymap.set('v', '>', '>gv')
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sh', builtin.git_status, { desc = '[S]earch changed [H]unks (git status)' })
      ...
      -- Fuzzy find across all files in project
      vim.keymap.set('n', '<leader>ss', function()
        builtin.grep_string {
          search = '',
        }
      end, { desc = '[SS] Fuzzily search all files in project' })
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
      ...
      -- python - disable ruff hover in favor of pyright
      vim.api.nvim_create_autocmd('LspAttach', {
  ...
  -- other colorschemes I like, might want to switch between
  { 'maxmx03/solarized.nvim' },
  {
    ...
    config = true,
    opts = {
      -- override default opts here: https://github.com/ellisonleao/gruvbox.nvim
      contrast = '', -- can be empty string, hard, or soft
    },
    init = function()
  ...
  -- Comment out code
  -- <count>gcc - comment out lines
  -- gc<motion> - comment out lines
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      -- require('mini.surround').setup() -- use nvim-surround instead
  ...
  { -- s-expression editing: https://github.com/snoe/vim-sexp
    'snoe/vim-sexp', -- This is a fork of guns/vim-sexp with the addition of not moving the cursor when slurping and barfing
    init = function()
      -- smart bracket and double quote matching stuff
      ...
      -- Most other terminals just support alt, which is an escape immediately followed by the key.
      --
      -- In Mac OS option+any letter key produces a special character. Here we just map those special characters to get the
      -- same effect for terminals without escape sequences.
      --
  ...
  -- it does jk by default, so add kj since I like to mash both keys simultaneously
  {
    ...
    opts = {
      ...
      -- s: select
      mappings = {
        i = { k = { j = '<Esc>:w<CR>' }, j = { k = '<Esc>:w<CR>' } },
        -- c = { k = { j = '<Esc>' } },
        t = { k = { j = '<C-\\><C-n>' }, j = { k = '<C-\\><C-n>' } },
        -- s = { k = { j = '<Esc>' } },
        -- v = { k = { j = '<Esc>' } },
      },
    },
